...
> **Note on test_AUC:** Due to class imbalance, 13 out of 206 species have zero positive examples in the test split after stratified splitting. ROC-AUC is undefined for these classes and excluded from the macro average. Test_AUC is therefore computed over 193/206 species — the 13 excluded species are the rarest in the dataset with too few total recordings to guarantee test set representation. Val_AUC covers a similar subset and is the primary metric used for model selection throughout training.
...
† test_AUC computed on seed=42 checkpoint only, evaluated over ~193/206 classes with ≥1 positive example in the test split. See note above.
...
Motivated by the hypothesis that the Transformer's modest advantage over the CNN baseline at 5s clips could be amplified with longer temporal context. Bird vocalizations often contain long-range structure (repeated phrases, response calls, harmonic progressions) that a 5-second window may truncate. Longer clips double the token count (~320 vs ~160 tokens), giving self-attention more temporal range to model these patterns. Due to increased memory requirements, batch size was reduced to 64 and LR scaled accordingly. Single seed (42) only — these runs are exploratory rather than part of the formal comparison. Batch size reduced to 64, LR scaled to 7.5e-5 to accommodate larger memory footprint.
...
| `cnn_transformer` (ResNet-18 + Transformer) | 0.9498 | 0.9465 | −0.003 |
...
† Evaluated over ~193/206 classes with ≥1 positive in the test split due to class imbalance. See note in primary results section.
